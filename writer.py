...
FilePath: \Bangumi\writer.py
'''

from request_json import get_subject_json
from multiprocessing import Process
from bangumi_queue import bangumiQueue
import json
...
class WriterProcess (Process):
    ...
    def __init__(self, beginID, endID):
    ...
    def run(self):
        for bangumiID in range(self.__beginID, self.__endID):
            ...
            if isinstance(subjectJsonDict, dict) and subjectJsonDict.get("code", None) == None and Bangumi.isWorthy(subjectJsonDict):
                # 如果读取到了一个字典，并且没有code一栏，视为有效数据
                # 我也不知道为什么bgm.tv设置成只有错误才返回错误码
                bangumiQueue.put(Bangumi(subjectJsonDict))
